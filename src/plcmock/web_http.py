...
def handler_class(dashboard: WebDashboardServer) -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        server_version = "PLCMockDashboard/0.4"
        ...
        def log_message(self, format: str, *args: Any) -> None:
            del format, args  # Polling the UI must not generate more UI logs.
        ...
        def _dispatch(self, method: str) -> None:
            try:
                ...
                self._bytes(HTTPStatus.OK, dashboard.asset(filename), content_type)
            except ApiError as exc:
                self._json(exc.status, {"ok": False, "error": exc.message})
            except (BrokenPipeError, ConnectionResetError):
                pass
            except Exception as exc:
                LOGGER.exception("web request failed method=%s path=%s", method, self.path, extra={"event": "web_request_failed"})
                self._json(HTTPStatus.INTERNAL_SERVER_ERROR, {"ok": False, "error": f"internal server error: {type(exc).__name__}"})

        def _api(self, method: str, path: str, query: Mapping[str, list[str]]) -> None:
            if method == "GET" and path == "/api/health":
                ...
                try:
                    minimum = None if not level or level.lower() == "all" else level_number(level.upper())
                ...
                self._json(
                    ...
                    dashboard.read_memory(
                        ...
                        _query_int(query, "count", 32, 1, dashboard.max_memory_points),
                    ),
                )
            elif method == "PUT" and path == "/api/memory":
                self._json(HTTPStatus.OK, dashboard.write_memory(self._body()))
            elif method == "POST" and path == "/api/logging":
                self._json(HTTPStatus.OK, {"ok": True, "logging": dashboard.set_log_mode(self._body().get("mode"))})
            else:
        ...
        def _body(self) -> Mapping[str, Any]:
            ...
            if not 0 < length <= 1024 * 1024:
                raise ApiError(HTTPStatus.BAD_REQUEST, "JSON request body is required and must be at most 1 MiB")
            try:
        ...
        def _json(self, status: HTTPStatus, payload: Mapping[str, Any]) -> None:
            self._bytes(status, json.dumps(payload, ensure_ascii=False, separators=(",", ":"), default=str).encode(), "application/json; charset=utf-8")
        ...
        def _headers(self, content_type: str | None, length: int) -> None:
            if content_type:
